...
const buildDocUrl = (doc, appId, action = 'view') => {
  ...
  if (direct) {
    ...
    const fullUrl = `https://localhost:8080${path}`
  ...
  if (doc.id && appId) {
    const fullUrl = `https://localhost:8080/api/v1/entities/${encodeURIComponent(appId)}/documents/${encodeURIComponent(doc.id)}/${action}`
