...
export const useNotificationStore = defineStore('notification', () => {
  ...
  const connectStream = () => {
    ...
    // Prefer token-based SSE so it works before cookies/auth fetches
    const token = localStorage.getItem('auth_token')
    const url = token ? `${baseURL}/notifications/stream-token?token=${token}` : `${baseURL}/v1/notifications/stream`
